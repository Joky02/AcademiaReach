...
# Constraints
- 论文数量：representative_papers 最多只列出 5 篇最具代表性的论文。若搜索结果中出现引用数（如 Cited by 123、被引用 123 次），必须提取到 citation_count，并优先按 citation_count 从高到低排序；没有明确引用数时才根据顶级 venue、主页高频出现、奖项/最佳论文、近期代表性和研究相关性综合排序。
